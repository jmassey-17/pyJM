...
for i in (0,1, 2): 
    if i == 0:
       ...
       # p.add_volume(mesh1, scalars='scalars', opacity = opacity, show_scalar_bar = False)
        ...
        (0, 0, 0),
        (-40, 1, 0)]
        p.save_graphic(f'{t}_FM_Domain_View_{i})_axis.svg')
        p.show()
        ...
        (0, 0, 0),
        (-40, 1, 0)]
        p.save_graphic(f'{t}_FM_Domain_View_{i}.svg')
        p.show()
    else: 
        p = pv.Plotter()
        #p.camera_position = 'xy'
