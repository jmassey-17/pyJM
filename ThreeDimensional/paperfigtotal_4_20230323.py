...
for key,c in zip(temps, colours):
    ...
    y = np.arange(len(x))/len(x)
    y = y[::-1]
    ax[2].errorbar(x/np.amax(x), y, xerr = abs(xe), c = c, fmt = 'o',markersize=8)
labels = [temp + 'K' for temp in temps]
ax[2].set_yticks([0,0.5,1])
#ax[2].legend(labels, bbox_to_anchor=(0, 0, 0.4, 1.01), frameon=True, framealpha = 1, edgecolor = 'none') #prop = {'size': 16},


#assymetries heat


ax[3].set_xlabel('$A/\phi_{Max}$')
